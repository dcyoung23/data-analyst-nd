...
def predictions(dataframe):
    ...
    dataframe['dayofweek'] = pd.DatetimeIndex(dataframe['DATEn']).dayofweek
    dummy_units = pd.get_dummies(dataframe['UNIT'], prefix='unit')
    dummy_hours = pd.get_dummies(dataframe['Hour'], prefix='hour')
    dummy_combined = dummy_units.join(dummy_hours)
    dataframe['dayofweek'] = pd.DatetimeIndex(dataframe['DATEn']).dayofweek
    dataframe['weekday'] = 0
    dataframe.loc[dataframe['dayofweek'] <= 4, 'weekday'] = 1

    features = dataframe[['weekday', 'rain', 'mintempi', 'fog']].join(dummy_combined)
...
if __name__ == "__main__":
    ...
    output_filename = "turnstile_data_master_with_weather_predict_gd.csv"
    ...
    print r_squared
    turnstile_master['ENTRIESn_hourly_predict'] = predicted_values
