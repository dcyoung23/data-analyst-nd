...
def lineplot_compare(hr_by_team_year_sf_la_csv):
    ...
    input_data = pd.read_csv(hr_by_team_year_sf_la_csv)
    #print input_data
    gg = ggplot(input_data,aes(x='yearID',y='HR',color='teamID')) + geom_point() + geom_line() + ggtitle('Total HRs by Year') + xlab('Year') + ylab('HR')
    return gg
