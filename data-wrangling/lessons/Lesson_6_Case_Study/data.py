...
POSITION = [ "lat", "lon"]
# define start characters of address dict
addr_pre = "addr:"
addr1 = set()
addr2 = set()
...
def shape_element(element):
    ...
    if element.tag == "node" or element.tag == "way" :
        ...
        for item in CREATED:
            if item in attributes:
                created_dict[item] = attributes[item]
        ...
        for tag in element.iter("tag"):
            check = tag.attrib['k']
            # ignore if tag has problem chars
            if problemchars.search(check):
                pass # do nothing
            # create address dict
            elif check[:5] == addr_pre and lower_colon.match(check):
                new_addr = check.replace(addr_pre,"")
                address_dict[new_addr] = tag.attrib["v"]
                #addr1.add(check)
            # ignore address fields with double colons
            elif check[:5] == addr_pre and not lower_colon.match(check):
                pass # do nothing
                #addr2.add(check)
            # all other include as normal
            else:
...
def process_map(file_in, pretty = False):
    ...
    file_out = "{0}.json".format(file_in.replace(".xml",""))
    data = []
